...
def run_ppc(fact, item_to_cat, gibbs_result, n_sim_draws=300):
    ...
    mu_i_draws = gibbs_result['mu_i']
    sigma2_draws = gibbs_result['sigma2']
...
if __name__ == "__main__":
    ...
    print("N obs:", len(fact), "N items:", len(item_to_cat), "N cats:", item_to_cat.max() + 1)

    print("\nFitting Normal hierarchical model...")
    ...
    print("\nsigma2 posterior mean -- Normal:", res_normal['sigma2'].mean(),
          " Student-t:", res_t['sigma2'].mean())
    ...
    # save gibbs results for reuse in part 3 (leave-some-out uses fresh refits, but keep normal mu_i for reference)
    np.save("/home/claude/mu_i_normal.npy", res_normal['mu_i'])
    np.save("/home/claude/mu_i_studentt.npy", res_t['mu_i'])
